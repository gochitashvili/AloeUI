...
<Steps>
  <Step>
    ...
    ```package-install
    npx shadcn@latest add "@aloeui/data-grid"
    ```
  </Step>
  <Step>
    ...
    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-select-column"
    ```

    [`DataGridSortMenu`](#datagridsortmenu):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-sort-menu"
    ```

    [`DataGridFilterMenu`](#datagridfiltermenu):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-filter-menu"
    ```

    [`DataGridRowHeightMenu`](#datagridrowheightmenu):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-row-height-menu"
    ```

    [`DataGridViewMenu`](#datagridviewmenu):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-view-menu"
    ```

    [`DataGridKeyboardShortcuts`](#datagridkeyboardshortcuts):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-keyboard-shortcuts"
    ```

    [`DataGridSkeleton`](#datagridskeleton):

    ```package-install
    npx shadcn@latest add "@aloeui/data-grid-skeleton"
    ```

    [`useDataGridUndoRedo`](#usedatagridundoredo):

    ```package-install
    npx shadcn@latest add "@aloeui/use-data-grid-undo-redo"
    ```
  </Step>
  <Step>
