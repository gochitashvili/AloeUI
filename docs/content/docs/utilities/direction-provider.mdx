---
title: Direction Provider
description: Provides bidirectional text support (RTL/LTR) across your application.
links:
  api: /docs/utilities/direction-provider#api-reference
  docs: https://radix-ui.com/docs/primitives/utilities/direction-provider
---

## Installation

### CLI

```package-install
npx shadcn@latest add @aloeui/direction-provider
```

### Manual

<Steps>
  <Step>
     Install the following dependencies:

     ```package-install
     @aloeui/direction-provider
     ```
  </Step>

  <Step>
    Copy and paste the following code into your project.

    <ComponentSource src="./registry/default/components/direction-provider.tsx" />
  </Step>
</Steps>

## Usage

```tsx
import { DirectionProvider } from "@aloeui/direction-provider"

export default function App() {
  return (
    <DirectionProvider dir="ltr">
      <YourApp />
    </DirectionProvider>
  )
}
```

## API Reference

### DirectionProvider

Manages direction context for the `useDirection` hook.

<AutoTypeTable
  path="./types/docs/utilities.ts"
  name="DirectionProviderProps"
/>

### useDirection

A hook to access the current direction.

```tsx
import { useDirection } from "@/components/direction-provider"

function Component() {
  const dir = useDirection()
  
  return (
    <Button dir={dir}>
      Do a kickflip
    </Button>
  )
}
```
